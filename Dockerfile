...
RUN apt-get update && apt-get install -y git zlib1g-dev libfreetype6-dev libjpeg62-turbo-dev libmcrypt-dev libpng-dev wget libldap2-dev libtidy-dev \
   ...
   && wget https://github.com/Jam3/BookStack/archive/master.tar.gz -O ${BOOKSTACK}.tar.gz \
   && tar -xf ${BOOKSTACK}.tar.gz && mv BookStack-master ${BOOKSTACK_HOME} && rm ${BOOKSTACK}.tar.gz  \
